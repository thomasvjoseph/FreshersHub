...
$id = getSession('logid');
 ...
 echo  $jobid=$_GET['jobid'];
  ...
  echo  $qury="select * from create_job where jobid=$jobid";
    $result=mysqli_query($conn,$qury) or die("error");
    $row=mysqli_fetch_assoc($result);
 
if(isset($_POST["submit"]))
    {
    echo $qury1="INSERT INTO applied_job(job_id,uid,status) VALUES($jobid,$id,1)";
      $obj=mysqli_query($conn,$qury1) or die("erorr");
      if($obj)
      {
        ?>
        <script>
        window.confirm("Are you sure To Apply");
...
.openIcon{
  ...
  <body>
     ...
     <!-- Sidebar -->
      ...
      <center>
        <h2 class="detail-title1">JOB INFO</h2>

        <br>
        <br>
  
  
<div style="width:800px; height:500px; border-width:800px; background-color:white; text-align:left; border-radius: 2px; box-sizing: border-box;">
  <p>
  <br id="id" ><b><?php echo $row['jobtitle'];?></b>
  <br id="id">Company :<?php echo $row['cname'] ?>
  <br id="id"><?php echo $row['jtype'] ?>
  <br id="id"><?php echo $row['city'] ?>
  <br id="id"><?php echo $row['jdescription'];?>
  <br id="id"><?php echo $row['qualification'] ?>
  <br id="id"><?php echo $row['ldate'] ?>
  <br id="id"><?php echo $row['company_info'] ?></p>
  <div class="col-md-12 col-sm-12 text-center ">
  
  <button  class="btn btn-success"  name="submit" type="submit">Apply Now</button>   </div>
</div>
